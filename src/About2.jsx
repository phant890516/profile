...
const callGemini = async (prompt, systemInstruction = "") => {
  try {
    ...
    console.error(error);
    return "ERROR: CONNECTION LOST. RETRY.";
  }
};
  ...
  return (
    <div className="group relative bg-[#0a0a0a] border border-[#333] hover:border-[#00f3ff] transition-all duration-300 p-6 flex flex-col items-center hover:-translate-y-2"
      ...
      {/* <button 
        ...
        disabled={loading}
        className="w-full mt-auto py-2 px-4 border border-[#333] bg-[#111] hover:bg-[#00f3ff]/10 hover:border-[#00f3ff] text-xs font-mono-cyber text-gray-400 hover:text-[#00f3ff] transition-all flex items-center justify-center gap-2 group/btn"
      >
        {loading ? <Loader2 className="animate-spin w-3 h-3"/> : <Sparkles className="w-3 h-3 text-[#ff00ff] group-hover/btn:animate-ping"/>}
        {loading ? 'GENERATING...' : 'GENERATE MISSION ✨'}
      </button> */}

      {/* Generated Mission Display */}
      {/* {mission && (
        <div className="mt-3 p-3 bg-[#00f3ff]/5 border border-[#00f3ff]/30 text-[10px] font-mono-cyber text-[#00f3ff] w-full animate-in fade-in slide-in-from-top-2">
          {mission}
        </div>
      )} */}
    </div>
...
// --- Sub-component: AI Chat Widget ---
const ChatWidget = ({ isOpen, setIsOpen }) => {
  const [messages, setMessages] = useState([
    { role: 'assistant', text: 'SYSTEM ONLINE. I AM LIN-AI. ASK ME ABOUT THE DEVELOPER.' }
  ]);
  ...
  const handleSend = async () => {
    ...
    const systemPrompt = `
      You are LIN-AI, a futuristic interface for Lin Hengyou's portfolio.
      User Data:
      - Name: Lin Hengyou
      - Born: 2000, Taiwan
      - Location: Osaka, Japan
      - School: HAL Osaka (Web Design/Development)
      - Skills: React, Node.js, Flask, WebGL, Shader, HTML, CSS, JS, Python.
      - Background: Used to do Maya/Blender 3D, now focused on Web.
      
      Tone: Robotic, Brief, Cyberpunk, Helpful. Use caps for keywords.
      Example: "USER QUERY ACKNOWLEDGED. LIN SPECIALIZES IN REACT."
    `;
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 flex flex-col items-end transition-all duration-300 ${isOpen ? 'w-80' : 'w-auto'}`}>
      
      {/* Chat Window */}
      {isOpen && (
        <div className="w-full bg-[#050505] border border-[#00f3ff] shadow-[0_0_20px_rgba(0,243,255,0.2)] mb-4 flex flex-col overflow-hidden h-96 animate-in slide-in-from-bottom-10 fade-in duration-300">
          {/* Header */}
          <div className="bg-[#00f3ff]/10 p-3 border-b border-[#00f3ff]/30 flex justify-between items-center">
            <span className="font-cyber text-xs text-[#00f3ff] flex items-center gap-2">
          ...
          {/* Messages */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 font-mono-cyber text-xs">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-2 border ${
                  m.role === 'user' 
            ...
            {loading && (
              <div className="text-[#00f3ff] animate-pulse">PROCESSING DATA...</div>
            )}
          </div>

          {/* Input */}
          <div className="p-2 border-t border-[#00f3ff]/30 bg-black flex gap-2">
            <input 
              type="text" 
              value={input}
              ...
              className="flex-1 bg-transparent border-none outline-none text-[#e0e0e0] text-xs font-mono-cyber placeholder-gray-700"
            />
            <button onClick={handleSend} disabled={loading} className="text-[#00f3ff] hover:text-white transition-colors">
              <Send size={16} />
            </button>
          </div>
        </div>
      ...
      <button 
        ...
        className={`
          ...
          bg-black border border-[#00f3ff] text-[#00f3ff] 
          font-cyber font-bold tracking-wider hover:bg-[#00f3ff] hover:text-black 
          transition-all duration-300 shadow-[0_0_15px_rgba(0,243,255,0.2)]
          ${isOpen ? 'bg-[#00f3ff] text-black' : ''}
        `}
      >
        <MessageSquare size={18} />
        {isOpen ? 'CLOSE TERMINAL' : 'SYSTEM HELP'}
      </button>
    </div>
  );
};
